...
def load_pdf_files():
    try:
        ...
        if os.path.isdir(DATA_PATH):
            ...
            raise CustomException(f"Invalid path: {DATA_PATH} is not a PDF file or directory.")

        if not documents:
            logger.warning("No PDFs were found.")
        else:
            logger.info(f"Successfully fetched {len(documents)} document(s)")

        return documents

    except Exception as e:
        logger.error(f"Failed to load PDF | Error: {e}")
        raise
